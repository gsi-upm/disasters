...
public class BuildStringData {
    ...
    private static final String URLRemoteServlet = "http://localhost:8090/HTTPservidor/ConsumeEvento"; // Ahora es una URL en un servidor de prueba pero mas tarde
                                                                // debera ser la URL del sevlet remoto(el que conecta con OCP) que recibe estos POST --> Suponiendo que su servlet para procesar register y produce sea uno y no dos distintos 
    private static final String URLServletConsume = "URL de nuestro servlet, el que procesa el POST de los partners"; // Este es el servlet local con el que nosotros consumimos los FirePlan que ellos nos manden.
        ...
        String dataToSend = "{\'"+param[2]+"-"+Integer.parseInt(param[0])+"\':" // Concatenamos la entidad y el id (ej: FireEvent-2)
                                        + "{"
                                            + "'type':"+"\'"+param[1]+"\'"   // Establece la funcion del POST. Toma dos valores "REGISTRAR" o "PRODUCIR".
                                                                        ...
                                                                        // "produce": Se está produciendo un evento.
                                            +",'size':"+"\'"+param[3]+"\'"
                                            +",'description':"+"\'"+param[4]+"\'"
                                            +",'name':"+"\'"+param[5]+"\'"
    ...
    public static String DataRegisterBuilder(String param[]) throws Exception{
        ...
        // "id="+Integer.parseInt(param[0]) --> En principio como nos queremos registrar a una entidad en genera (FirePlan) y no a la de un evento en particular (FirePlan-2) no hace falta mandar la id.  
