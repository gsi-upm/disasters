...
public class ColaborarEvacuarSanosPlan extends EnviarMensajePlan{
	...
	public void body(){
		...
		env.printout("XX auxiliar: evacuando la residencia", 2, 0, true);
		...
		try{
			...
			env.printout("XX auxiliar: de " + sanos.length() + " residentes sanos me encargo de evacuar a " +
					(sanos.length()-numeroEvacuadosSanos), 2, 0, true);
			...
			env.printout("XX auxiliar: todos los residentes a cargo del auxiliar evacuados!!", 2, 0, true);
	...
	private void evacuar(JSONObject persona){
		...
		try{
			...
			env.printout("XX auxiliar: evacuando al anciano " + id, 2, 0, true);
