...
public class BomberoEnDesastrePlan extends Plan {
	...
	public void body() {
		...
		// Posición actual del bombero, que le permite recoger al herido.
		...
		if (atrapados != null) {
			System.out.println("^^ firemen: He encontrado un herido atrapado!! cuya id es: "
							+ atrapados.getId());
		...
		try {
			env.andar(getAgentName(), posicionActual, destino, env.getAgent(
					getAgentName()).getId(), 0);
			// el cuarto parámetro del método andar es el id del bichito
			// que queremos transportar. en bombero es siempre 0
		} catch (Exception ex) {
			System.out.println("^^ firemen: Error método andar: " + ex);
		}
		...
		while(des.getSlight()!=null||des.getSerious()!=null||des.getDead()!=null){
			//System.out.println("^^ firemen: No puedo marcharme porque quedan heridos, espero un poco más...");
			waitFor(2000);
			des = env.getEvent(idDes);
		}
